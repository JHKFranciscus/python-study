...
@app.route("/")
def home():
    return render_template("app1_fixed.html")
...
def edit_record(record_id):
    ...
    if "subject" in update_record and update_record["subject"] not in allowed_subject:
        return jsonify(
            {"error": "invalid subject"}
        ), 400

    allowed_fields = ["subject", "study_date", "minutes", "memo"]

    update_data = {}

    for field in allowed_fields:
        if field in update_record:
            update_data[field] = update_record[field]
