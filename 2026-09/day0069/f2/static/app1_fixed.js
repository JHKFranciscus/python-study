    ...
    fetch(url)
        ...
        .then(data => {
            ...
            for (const record of records_list) {
                ...
                p1.textContent = `subject: ${record.subject}`;
                p2.textContent = `study_date: ${record.study_date}`;
                p3.textContent = `minutes: ${record.minutes}`;
                p4.textContent = `memo: ${record.memo}`;
                ...
                div2.appendChild(updatechosenButton);
...
function preupdateRecord(record) {
    updateSubjectSelector.value = record.subject
    updateStudyDateSelector.value = record.study_date
    updateMinutesSelector.value = Number(record.minutes)
    updateMemoSelector.value = record.memo
